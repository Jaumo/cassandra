...
public class StreamingTransferTest
{
    ...
    {
        StreamResultFuture futureResult = new StreamPlan(StreamType.TEST_TRANSFER).execute();
        ...
        Futures.addCallback(futureResult, new FutureCallback<StreamState>()
        {
            public void onSuccess(StreamState result)
            {
                ...
                assert result.type == StreamType.TEST_TRANSFER;
    ...
    public void testRequestEmpty() throws Exception
    {
        ...
        StreamResultFuture futureResult = new StreamPlan(StreamType.TEST_TRANSFER)
    ...
    private void transferRanges(ColumnFamilyStore cfs) throws Exception
    {
        ...
        StreamPlan streamPlan = new StreamPlan(StreamType.TEST_TRANSFER).transferRanges(LOCAL, cfs.keyspace.getName(), ranges, cfs.getTableName());
    ...
    {
        StreamPlan streamPlan = new StreamPlan(StreamType.TEST_TRANSFER).transferFiles(LOCAL, makeStreamingDetails(ranges, Refs.tryRef(Arrays.asList(sstable))));
        streamPlan.execute().get();
        verifyConnectionsAreClosed();
