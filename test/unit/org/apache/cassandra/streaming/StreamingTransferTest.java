...
public class StreamingTransferTest
{
    ...
    {
        StreamResultFuture futureResult = new StreamPlan(StreamOperation.UNIT_TEST).execute();
        ...
        Futures.addCallback(futureResult, new FutureCallback<StreamState>()
        {
            public void onSuccess(StreamState result)
            {
                ...
                assert result.streamOperation == StreamOperation.UNIT_TEST;
    ...
    public void testRequestEmpty() throws Exception
    {
        ...
        StreamResultFuture futureResult = new StreamPlan(StreamOperation.UNIT_TEST)
    ...
    private void transferRanges(ColumnFamilyStore cfs) throws Exception
    {
        ...
        StreamPlan streamPlan = new StreamPlan(StreamOperation.UNIT_TEST).transferRanges(LOCAL, cfs.keyspace.getName(), ranges, cfs.getTableName());
    ...
    {
        StreamPlan streamPlan = new StreamPlan(StreamOperation.UNIT_TEST).transferFiles(LOCAL, makeStreamingDetails(ranges, Refs.tryRef(Arrays.asList(sstable))));
        streamPlan.execute().get();
        verifyConnectionsAreClosed();
