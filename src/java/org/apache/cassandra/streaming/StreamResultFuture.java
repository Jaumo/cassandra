...
public final class StreamResultFuture extends AbstractFuture<StreamState>
{
     ...
     */
    private StreamResultFuture(UUID planId, StreamType type, StreamCoordinator coordinator)
    {
        this.planId = planId;
        this.type = type;
    ...
    private StreamResultFuture(UUID planId, StreamType type, boolean keepSSTableLevels, boolean isIncremental, UUID pendingRepair)
    {
        this(planId, type, new StreamCoordinator(0, keepSSTableLevels, isIncremental, new DefaultConnectionFactory(), false, pendingRepair));
    }

    static StreamResultFuture init(UUID planId, StreamType streamType, Collection<StreamEventHandler> listeners,
                                   StreamCoordinator coordinator)
    {
        StreamResultFuture future = createAndRegister(planId, streamType, coordinator);
        if (listeners != null)
        {
        ...
        logger.info("[Stream #{}] Executing streaming plan for {}", planId,  streamType.toString());
                                                                    ...
                                                                    UUID pendingRepair) throws IOException
    {
        ...
        if (future == null)
        {
            logger.info("[Stream #{} ID#{}] Creating new streaming plan for {}", planId, sessionIndex, type.toString());

            // The main reason we create a StreamResultFuture on the receiving side is for JMX exposure.
            future = new StreamResultFuture(planId, type, keepSSTableLevel, isIncremental, pendingRepair);
            StreamManager.instance.registerReceiving(future);
        }
        future.attachConnection(from, sessionIndex, connection, isForOutgoing, version);
        logger.info("[Stream #{}, ID#{}] Received streaming plan for {}", planId, sessionIndex, type.toString());
        return future;
    }

    private static StreamResultFuture createAndRegister(UUID planId, StreamType type, StreamCoordinator coordinator)
    {
        StreamResultFuture future = new StreamResultFuture(planId, type, coordinator);
    ...
    {
        return new StreamState(planId, type, coordinator.getAllSessionInfo());
    }
